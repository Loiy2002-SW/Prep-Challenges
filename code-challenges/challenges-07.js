...
const objLat = (obj) => {
    ...
    let fistName = obj.firstName[0].toUpperCase() + obj.firstName.substring(1);
    let lastName = obj.lastName[0].toUpperCase() + obj.lastName.substring(1);

    return `my name is ${fistName} ${lastName} I am ${obj.age} YO, and I love ${obj.hobby}.`;
...
const cvFormatter = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (arr[i].yearsOfExperience > 1) {
            ...
            filteredArray.push(
                {
                    fullName: userFullName(arr[i].firstName, arr[i].lastName),
                    tech: arr[i].tech
                }
            );

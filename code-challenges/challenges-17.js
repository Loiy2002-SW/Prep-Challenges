...
//
    ...
    const recursionPattern = (int1, int2, direction = -1, arr = [int1]) => {
        
        if (arr[arr.length - 1] === int1 && direction === 1) {
            return arr;
        }
    
       
        const nextNum = arr[arr.length - 1] + (int2 * direction);
    
        if (nextNum < 0 && direction === -1) {
            direction = 1;
        }
        ...
        return recursionPattern(int1, int2, direction, arr);
    };
...
const filterLinks = (str) => {
    ...
    let startOfTheLink = str.indexOf('href')+13;
    let endOfTheLink = str.indexOf('.com');

    

    if(str.includes('.org')){
         endOfTheLink = str.indexOf('.org');
    }else if(str.includes('.net')){
         endOfTheLink = str.indexOf('.net');
    }



    let reslut = str.substring(startOfTheLink,endOfTheLink+4);
...
const isPalindrome = (str) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if(/^[A-Za-z]$/.test(arr[i])){
            ...
            formattedArr.push(arr[i].toLowerCase());

        }
    }

    let array = formattedArr.slice('');

    formattedArr.reverse();

    if(arraysEqual(array, formattedArr)){
        return true;
    }else{
        return false;
    }

...
const replaceWords = (str) => {
    ...
    let toBeReplaced = ["I", "was", "am"];
    let replaceWith = ["We", "were", "are"];

    for (let i = 0; i < words.length; i++) {
        
        let index = toBeReplaced.indexOf(words[i]);
        if (index !== -1) {
            // Replace the word using splice
            words.splice(i, 1, replaceWith[index]);
        }
    }

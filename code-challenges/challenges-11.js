...
function fullName(arr) {
    // write your code here
    let fullNamesArray = arr.map((student) => `${student.firstName} ${student.lastName}`);
...
function gradesAvg(arr) {
    ...
    let newArray = arr.map((student) => {
        ...
        let sum = 0;
        student.gradsList.map((grade) => sum += grade);

        return {
            firstName: student.firstName,
            lastName: student.lastName,
            gradsList: student.gradsList,
            avg: sum/ student.gradsList.length
        }
    }
);

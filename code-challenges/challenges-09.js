...
const employeesBonus = (arr) => {
    ...
    arr.forEach(element => {
        ...
        let s = element.salary +'';
         s = s.substring(0, s.length-1);
        

        if(element.workHours > 8){
            result.push({name: element.name, section: element.section, workHours: element.workHours, salary: (parseFloat(s)+100) + '$'});
        }else if( element.workHours <= 8){
            result.push({name: element.name, section: element.section, workHours: element.workHours, salary: (parseFloat(s)+50) + '$'});
        }else {
            result.push({name: element.name, section: element.section, workHours: element.workHours, salary: element.salary});
        }
